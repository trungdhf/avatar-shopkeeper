...
To try VRoid Project's greeting or full-body spin, download the [free seven-motion pack](https://booth.pm/ja/items/5512385) from BOOTH, extract `VRMA_02.vrma` (Greeting) or `VRMA_05.vrma` (Spin) and select it with **Try an official VRoid motion** on the shop page. The file is parsed in your browser and never uploaded. BOOTH's license permits use and testing but prohibits redistribution of extractable motion files; this project does not include or host either file. Character animation credits to pixiv Inc.'s VRoid Project.
